...
namespace NuitrackSDK.NuitrackDemos
{
    public class UserTrackerVisMesh : MonoBehaviour
    {
        ...
        private void Initialize()
        {
            ...
            InitMeshes(
                xRes, yRes,
                (mode.Intrinsics.FX / mode.XRes),
                (mode.Intrinsics.FY / mode.YRes),
                (mode.Intrinsics.CX / mode.XRes),
                (mode.Intrinsics.CY / mode.YRes));
        }

        void InitMeshes(int cols, int rows, float fx, float fy, float cx, float cy)
        {
            meshMat.SetFloat("fX", fx);
            meshMat.SetFloat("fY", fy);
            meshMat.SetFloat("cX", cx);
            meshMat.SetFloat("cY", cy);
